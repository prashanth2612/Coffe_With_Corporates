...
export default function UpdateForm({ config, formElements, withUpload = false }) {
  ...
  useEffect(() => {
    if (current) {
      ...
      // DatePicker requires dayjs objects, NOT formatted strings
      if (newValues.birthday) newValues.birthday = dayjs(newValues.birthday);
      if (newValues.date) newValues.date = dayjs(newValues.date);
      if (newValues.expiredDate) newValues.expiredDate = dayjs(newValues.expiredDate);
      if (newValues.created) newValues.created = dayjs(newValues.created);
      if (newValues.updated) newValues.updated = dayjs(newValues.updated);
